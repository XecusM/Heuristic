'''
This file is for Particle Swarm modules it has all functions needed
for the Algorithm.
...
NOTES:
Particle Swarm always want the minimum fitness value so take care
if you want the max. make the fitness negative values
'''
import Fo
import GA
import random
...
def swarm(PSO_population, variables_limits, method_parameters):
    ...
    This the main function for Particle Swarm Algorithm it needs
        1. dictionary of population list and
            best_chromosome dictionary (PSO_population)
        2. a list of tuples each tuple has the minimum,
            maximum and varible type (variables_limits)
        3. dictionary of (method_parameters) contain inertia weight,
            cognative and social constants
    ...
    if 'velocity' not in population[0]:
        ...
        # sort population by fitness to get the best chromosome
        population = GA.sort_population(population)
        # check if the best_chromosome better than
        # the population best chromosome
...
def intial_velocity(population, variables_limits):
    '''
    This function is to add the key of velocity for
    the population with its initial values it needs the population
    and variables_limits to return population have the initial velocities
    '''
...
def update_velocity(
    ...
    it needs the population, the best chromosome, method parameters
    and variables limits
    returns the population with its updated velocities
    '''
    ...
    for i in range(0, len(population)):
        for j in range(0, len(population[i]['gene'])):
            ...
            velocity_cognative = c1 * r1 \
                * (population[0]['gene'][j] - population[i]['gene'][j])
            # calculate velocity of social
            velocity_social = c2 * r2 \
                * (best_chromosome['gene'][j] - population[i]['gene'][j])
            # check if the variables wheather float of integer
            if variables_limits[j][2] == 'float':
                population[i]['velocity'][j] = w \
                    * population[i]['velocity'][j] \
                    + velocity_cognative + velocity_social
            else:
                population[i]['velocity'][j] = int(
                    w * population[i][
                        'velocity'][j] + velocity_cognative + velocity_social)
    return population


def update_position(population, variables_limits):
    '''
    This function gives the new positions for every particle
    by getting the population and variables limits
    It returns the populaiton with the new positions
    '''
    for i in range(0, len(population)):
        for j in range(0, len(population[i]['gene'])):
            # calculate the new position
            population[i]['gene'][j] = population[i]['gene'][j] \
                + population[i]['velocity'][j]
        ...
        population[i] = Fo.objective_function(population[i])
    return population
