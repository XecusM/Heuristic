...
NOTES:
Genatic Algorithm always want the minimum fitness value so take care if you want the max. make the fitness negative values
'''
...
def chrom_selection(population,selection_method='fitness'):
    '''
    This function need the population as a list of chromosomes and returns a random chromosome
    '''
    #slection acording to method defined
    if selection_method=='uniform':
        ...
        for chromosome in population:
            ...
            #check if the random number is fallen between the chromosome rank and the next chromosome
...
def survive_population(population,pop_size):
    '''
    This function for exclude the worse chromosomes from the population and return the population with its initial size
    '''
    #sort population
...
def cross_over(population,variables_limits,method_parameters):
    '''
    This function create two new chromosomes from two random chromosomes selected from the given population.
    ...
    if method_parameters['cross_method']=='new_generation':
        ...
        while len(out_pop)<len(population):
            ...
            chrom1=chrom_selection(population,method_parameters['selection_method'])
            chrom2=chrom_selection(population,method_parameters['selection_method'])
            ...
            if random.random()<=method_parameters['mutation_prob']:
                chrom1=mutation(chrom1,variables_limits,method_parameters['mutation_prob'])
            #mutate chrom2
            if random.random()<=method_parameters['mutation_prob']:
                chrom2=mutation(chrom2,variables_limits,method_parameters['mutation_prob'])
            #calculate chromosomes fitness
            chrom1=objective_function(chrom1)
            chrom2=objective_function(chrom2)
...
def mutation(chromosome,variables_limits,probability=0.5):
    '''
    This function mutate one or more paramters of the given chromosome according to given probability
    ...
    for i in range(len(chromosome['gene'])):
        if random.random()<=probability:
            if variables_limits[i][2]=='integer':
                chromosome['gene'][i]=random.randint(variables_limits[i][0],variables_limits[i][1])
            elif variables_limits[i][2]=='float':
                chromosome['gene'][i]=random.uniform(variables_limits[i][0],variables_limits[i][1])
...
def no_deplication(chromosome, population):
    '''
    This function to check if the chromosome exist in the new population or not
    '''
...
def stop_time(Stop_minutes=0,time_type='current'):
    '''
    This function to determine the stop time in minutes and return it or the current time
    '''
    if time_type=='current':
        # get the current time in seconds
        now=datetime.datetime.now().day*24*60*60+datetime.datetime.now().hour*60*60+datetime.datetime.now().minute*60+datetime.datetime.now().second
        ...
        endtime=endtime.day*24*60*60+endtime.hour*60*60+endtime.minute*60+endtime.second
