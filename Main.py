'''
This is the main program to control the Heuristic Techniques and the objective function
'''
from GA import *
from PSO import *
...
population=initial_population(settings['nvariables'],settings['population_size'],settings['variables_limits'])
# begin Calculating
print('Calculating...')
m=0
...
for method in settings['method']:
    ...
    if method=='GA':
        if settings['stopping'][m][1]=='minutes':
            ...
            end_time=stop_time(settings['stopping'][m][0],'end')
            while end_time>stop_time():
                #create a new population after cross over and mutation
                population=cross_over(population,settings['variables_limits'],settings['method_parameters'][m])
                i+=1
    ...
    # Particle Swarm Algorithm
        ...
        best_chromosome=random_chrom(settings['nvariables'],settings['variables_limits'])
        # fill up the PSO_population for swarm Calculations
        PSO_population={'best_chromosome':best_chromosome,'population':population}
        if settings['stopping'][m][1]=='minutes':
            i=1
            #get the end time in minutes
            end_time=stop_time(settings['stopping'][m][0],'end')
            while end_time>stop_time():
                ...
                PSO_population=swarm(PSO_population,settings['variables_limits'],settings['method_parameters'][m])
                i+=1
        elif settings['stopping'][m][1]=='iterations':
            for i in range(settings['stopping'][m][0]):
                #get the new population after swarming
